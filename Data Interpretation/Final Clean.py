...
dead_line = []
for line in ac:
    if not line.strip().endswith("click to expand..."): #there was a problem where sometimes the question was too long and it would overwrite the answer. These answers would end with "click to continue" and this program just removes them from the file
        if "click to expand..." in line:
            parts = line.split("expand...", 1) #sometimes there would be an issue where the question was long enough to interupt the answer and appear inside the answer, so this just removes anything before the "click to expand"
            line = parts[1]
