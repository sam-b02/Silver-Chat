...
def get_max_page_number(soup):
    ...
    if page_items:
        ...
        max_page_number = int(
            last_page_href.split("/")[-1].split("?")[0].replace("page-", "")
        )
        if max_page_number > 100:
            max_page_number = (
                100  # only extracts 100 pages at max; if less, that number
            )
...
with open(r"base data\questions.txt", "a", encoding="utf-8") as questions_file, open(
    r"base data\answers.txt", "a", encoding="utf-8"
) as answers_file:
    for count in range(0, 15):  # increments through each page
        response = requests_retry_session().get(
            urls[count - 1] + "1" + url2, timeout=10
        )  # creates a requests session, with retry logic
        soup = BeautifulSoup(response.text, "html.parser")
        max_page = get_max_page_number(
            soup
        )  # gets the max number of pages it should increment over

        for i in range(0, max_page + 1):
            time.sleep(0.5)  # sleep for less server stress
            url = (
                urls[count - 1] + str(i) + url2
            )  # combines the first part of the url with the page number and the second part of the url
            response = requests_retry_session().get(
                url, timeout=10
            )  # another requests session to check if page is up
            if response.status_code == 200:  # if page is up
                html_content = response.text
                soup = BeautifulSoup(html_content, "html.parser")
                thread_links = soup.find_all(
                    "a", attrs={"data-xf-init": "preview-tooltip"}
                )  # scrapes the list of all the posts on one forum page
                counter = 1
                for link in thread_links:
                    thread_url = "https://forums.tomshardware.com" + link["href"]
                    thread_response = requests_retry_session().get(
                        thread_url, timeout=5
                    )  # response session for each forum post

                    if thread_response.status_code == 200:
                        thread_soup = BeautifulSoup(thread_response.text, "html.parser")
                        title = thread_soup.find("h1").text.strip()  # scrapes the title

                        question_content = thread_soup.find(
                            "div", {"class": "bbWrapper"}
                        ).text.strip()  # scrapes the question
                        bb_wrappers = thread_soup.find_all(
                            "div", {"class": "bbWrapper"}
                        )  # scrapes the answer

                        if bb_wrappers and len(bb_wrappers) > 1:
                            answer_content = bb_wrappers[
                                1
                            ].text.strip()  # cleans the answer a little bit, chooses the first answer (the one that was marked as most helpful)
                            answers_file.write(title + "\n" + answer_content + "\n")
                            answers_file.write(
                                f"------------------------------------------------count is {count} and i is {i} and counter is {counter}\n"
                            )
